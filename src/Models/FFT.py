'''
Implements Full Fine-Tuning - Entire backbone trained + classifier (so all parameters trained)
'''
...
def write_summary_header(path: str):
    with open(path, "w", encoding="utf-8") as f: #open file in write mode and writes to file..
        f.write("FFT SEED SWEEP SUMMARY\n")
...
def build_fft_model(num_classes: int, dropout_p: float):
    ...
    print(f"\n=== Parameter Summary (MNV2-0.35 FFT) ===")
...
def run_fft_seed_sweep():
    sweep_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    sweep_dir = os.path.join(SAVE_DIR, f"FFT_seed_sweep_{sweep_stamp}")
    ...
    for seed in SEEDS:
        ...
        # Tee terminal output to per-seed log
        ...
        try:
            print("=" * 80)
            print(f"FFT RUN | seed={seed} | device={DEVICE}")
            print(f"Config: LR={LR} dropout={DROPOUT_P} batch={BATCH_SIZE} img={IMG_SIZE}")
            print("Sizes:", dataset_sizes)
            print("Num classes:", num_classes)
            print("=" * 80)

            model = build_fft_model(num_classes=num_classes, dropout_p=DROPOUT_P)
            ...
            scheduler_fft = ReduceLROnPlateau(
                optimizer,
                mode="min",
                factor=0.1,
                patience=5,
            ...
            for epoch in range(1, MAX_EPOCHS + 1):
                ...
                scheduler_fft.step(val_loss)

                elapsed = time.time() - start_time
                print(f"[FFT] Train loss: {train_loss:.4f}  acc: {train_acc:.4f}")
                print(f"[FFT] Val   loss: {val_loss:.4f}  acc: {val_acc:.4f}")
                print(f"[FFT] Elapsed time: {elapsed/60:.1f} min")
                ...
                if val_acc > best_val_acc + 1e-4:
                    ...
                    best_model_wts = copy.deepcopy(model.state_dict())
                    epochs_no_improve = 0
                    print(f"  -> New best FFT val_acc: {best_val_acc:.4f} at epoch {best_epoch}")
                else:
                    epochs_no_improve += 1

                if epochs_no_improve >= PATIENCE:
                    print(f"[FFT] Early stopping triggered at epoch {epoch}")
                    break
            ...
            print(f"\nBest FFT model from epoch {best_epoch} with val_acc={best_val_acc:.4f}")

            # Save best fp32 model (per-seed run folder)
            weights_path = os.path.join(
                run_dir,
                f"FFT-Seed{seed}-FP32_bestVal{best_val_acc:.4f}.pth"
            )
            torch.save(model.state_dict(), weights_path)
            print(f"Saved best FP32 FFT model to: {weights_path}")
            ...
            # Restore stdout/stderr and close per-seed log
            ...
            # Cleanup
            try:
                del model, optimizer, scheduler_fft, criterion
            except Exception:
                pass
    ...
    with open(summary_txt, "a", encoding="utf-8") as f:
        ...
        f.write(f"TEST f1 : {tfmu:.6f} ± {tsd:.6f}\n\n")
