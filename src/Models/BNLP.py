'''
Implements Batch Normalistion Tuning - Only BN affine parameters trained
'''
...
def write_summary_header(path: str):
    with open(path, "w", encoding="utf-8") as f: #open file in write mode and writes to file..
        f.write("BNLP SEED SWEEP SUMMARY\n")
...
def build_bnlp_model(num_classes: int, dropout_p: float):
    ...
    # Replace classifier with dropout + num_classes-way linear head
    base.classifier = nn.Sequential(
        nn.Dropout(p=dropout_p),
        nn.Linear(in_features, num_classes)
    )

    # ---- BN tuning: freeze conv weights, train BN affine + head ----

    # Freeze all parameters
    for p in base.parameters():
        p.requires_grad = False

    # Unfreeze BN affine parameters (γ, β)
    bn_trainable_params = 0
    for m in base.modules():
        if isinstance(m, (nn.BatchNorm2d, nn.BatchNorm1d)): #loops over all modules and find BN layers that have BN parameters to update
            if m.weight is not None: # set BN weight as True (can be trained)
                m.weight.requires_grad = True
                bn_trainable_params += m.weight.numel()
            if m.bias is not None:
                m.bias.requires_grad = True # set BN bias as True (can be trained)
                bn_trainable_params += m.bias.numel()

    # Unfreeze classifier head like LP
    ...
    head_params        = sum(p.numel() for p in base.classifier.parameters())
    backbone_params    = total_params - head_params

    print(f"\n=== Parameter Summary (MNV2-0.35 BNLP, SGD) ===")
    print(f"Backbone params (total):     {backbone_params:,}")
    print(f"BN params (trainable):       {bn_trainable_params:,}")
...
# Function that runs the entire BNLP experiment across all seeds
def run_bnlp_seed_sweep():
    sweep_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    sweep_dir = os.path.join(SAVE_DIR, f"BNLP_seed_sweep_{sweep_stamp}") #so every run gets unique folder
    ...
    for seed in SEEDS:
        ...
        try:
            print("=" * 80)
            print(f"BNLP RUN | seed={seed} | device={DEVICE}")
            ...
            model = build_bnlp_model(num_classes=num_classes, dropout_p=DROPOUT_P)

            #iterates through params, only trains the params we set true above (BN stats, Classifier)
            ...
            scheduler_bn = ReduceLROnPlateau(
                optimizer,
                mode="min",
                factor=0.1,
                patience=5,
            ...
            for epoch in range(1, MAX_EPOCHS + 1):
                ...
                # ---- Train ----
                # BN stays in train mode so running_mean/var are recalibrated on PV
                model.train()
            ...
            weights_path = os.path.join(
                run_dir,
                f"BNLP-Seed{seed}-FP32_bestVal{best_val_acc:.4f}.pth"
            )
            torch.save(model.state_dict(), weights_path)
            print(f"Saved best FP32 BNLP model to: {weights_path}")
            ...
            # Cleanup
            try:
                del model, optimizer, scheduler_bn, criterion
            except Exception:
                pass
