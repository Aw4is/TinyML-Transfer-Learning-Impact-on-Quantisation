'''
Implements Logit Standardisation Knowledge Distillation
Uses Full Fine-Tuning with a ResNet50 teacher to guide the MNV2-0.35 student
Inspired by: https://arxiv.org/abs/2403.01427
'''
...
from src.Models.KD.Distiller.KD import kd_loss
...
TEACHER_CKPT   = r"teacherweights"
STU_WEIGHTS    = r"pathtoimagenetweights"
...
def strip_module_prefix(state_dict: dict) -> dict:
    """
    If a model was saved with DataParallel, keys may be prefixed with 'module.'.
    This removes it so strict loading works.
    """
...
def build_teacher(num_classes: int):
    """
    Build ResNet50 teacher and load plant-dataset checkpoint.
    num_classes is passed in explicitly so we don't rely on globals.
    """
    print("Building ResNet50 teacher...")

    # No need for pretrained weights here – the checkpoint will overwrite them.
    ...
    if isinstance(ckpt, dict):
        if "model" in ckpt:
            ...
            best_top1 = ckpt.get("best_top1", "N/A")
            epoch = ckpt.get("epoch", "N/A")
            print(f"Loaded teacher checkpoint dict: epoch={epoch}, best_top1={best_top1}")
        elif "model_state_dict" in ckpt:
            state_dict = ckpt["model_state_dict"]
            print("Loaded teacher checkpoint dict: key=model_state_dict")
        else:
            # Sometimes checkpoints are saved as state_dict directly inside a dict-like wrapper.
            # If it doesn't match, we still try to treat it as state_dict.
            state_dict = ckpt
            print("Loaded teacher checkpoint dict: using entire dict as state_dict (fallback)")
    else:
        state_dict = ckpt
...
def build_student(num_classes: int, dropout_p: float):
    print("Building MobileNetV2 (width_mult=0.35) student with Li et al. weights...")
    student = MobileNetV2(num_classes=1000, width_mult=0.35)
    state = torch.load(STU_WEIGHTS, map_location="cpu")
    student.load_state_dict(state, strict=True)
    print("Pretrained student weights loaded.")
    ...
    print(f"\n=== KD Student Summary (MobileNetV2-0.35) ===")
...
# Function that runs the entire KD experiment  across all seeds
def run_kd_seed_sweep():
    sweep_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    sweep_dir = os.path.join(SAVE_DIR, f"KD_seed_sweep_{sweep_stamp}") #so every run gets unique folder
    ...
    for seed in SEEDS:
        ...
        try:
            ...
            # build teacher + student and define optimiser
            teacher = build_teacher(num_classes=num_classes)
            student = build_student(num_classes=num_classes, dropout_p=DROPOUT_P)
            # --- Quick teacher eval (once per seed) ---
            ...
            scheduler_kd = ReduceLROnPlateau(
                optimizer,
                mode="min",
                factor=0.1,
                patience=5,
            ...
            for epoch in range(1, MAX_EPOCHS + 1):
                ...
                for inputs, labels in dataloaders["train"]:
                    ...
                    # KD loss (Sun-style logit standardisation)
                ...
                with torch.no_grad():
                    for inputs, labels in dataloaders["val"]:
                        ...
                        loss_ce = criterion_ce(outputs, labels)  # CE only for scheduler
                        _, preds = torch.max(outputs, 1)

                        val_loss += loss_ce.item() * inputs.size(0)
                        val_corrects += torch.sum(preds == labels)

                val_loss /= dataset_sizes["val"]
                val_acc  = val_corrects.double().item() / dataset_sizes["val"]

                scheduler_kd.step(val_loss)
            ...
            weights_path = os.path.join(
                run_dir,
                f"KD-Seed{seed}-FP32_bestVal{best_val_acc:.4f}.pth"
            )
            torch.save(student.state_dict(), weights_path)
            print(f"Saved best FP32 KD model to: {weights_path}")

            # Evaluate val/test and write clean files
            val_m  = evaluate_split(student, dataloaders["val"],  DEVICE, "val",  val_txt,  class_names)
            ...
            # Cleanup
            try:
                del teacher, student, optimizer, scheduler_kd, criterion_ce
            except Exception:
                pass
